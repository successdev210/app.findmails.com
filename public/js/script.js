$(document).ready(function(){
	...
	$('.finder__search').on("submit" ,function(e){
		...
		if (errors == 0) {
			...
			// $.get('http://167.235.254.95:8765/emailfinder/', 
			// 	{
			// 		first_name: "elon",
			// 		last_name: "musk",
			// 		domain_name: "tesla"
			...
			setTimeout(function(){
				...
				$('.company__elem').css("display" ,'none');
				if ($(currentForm).find(".name").val().toLowerCase() == "elon musk" && $(currentForm).find(".company").val().toLowerCase() == "tesla") {
					// console.log($('.company__elem'));
